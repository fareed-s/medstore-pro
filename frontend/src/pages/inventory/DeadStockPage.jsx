    ...
    <div>
      ...
      <div className="flex gap-1 mb-4 bg-gray-100 rounded-xl p-1 w-fit">
        {tabs.map(tab => (
          <button key={tab.key} onClick={() => setActiveTab(tab.key)}
            className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === tab.key ? 'bg-white shadow-sm text-primary-700' : 'text-gray-500 hover:text-gray-700'}`}>
            <tab.icon className="w-4 h-4" /> {tab.label}
          </button>
        ))}
        {activeTab === 'dead' && (
          <select className="ml-2 text-sm border border-gray-200 rounded-lg px-2" value={days} onChange={(e) => setDays(parseInt(e.target.value))}>
          ...
          <div className="overflow-x-auto max-h-[60vh] overflow-y-auto">
            <table className="w-full text-sm">
              <thead className="sticky top-0 bg-white"><tr className="table-header">
                ...
                <th className="px-4 py-2 text-right">Stock</th><th className="px-4 py-2 text-right">Cost Value</th>
                <th className="px-4 py-2 text-right">Retail Value</th><th className="px-4 py-2">Rack</th>
              </tr></thead>
              <tbody className="divide-y divide-gray-50">
                {deadData.items.map(item => (
                  <tr key={item._id} className="hover:bg-gray-50/50">
                    <td className="px-4 py-2"><p className="font-medium text-xs">{item.medicineName}</p><p className="text-[10px] text-gray-400">{item.genericName}</p></td>
        ...
        <div>
          <div className="card mb-4">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={fastMovers.slice(0, 10).map(m => ({ name: m.medicineName?.substring(0, 15), qty: m.totalQty }))}>
          ...
          <div className="card overflow-hidden p-0">
            <table className="w-full text-sm">
              <thead><tr className="table-header">
                <th className="px-4 py-2">#</th><th className="px-4 py-2">Medicine</th><th className="px-4 py-2 text-right">Qty Sold</th>
                <th className="px-4 py-2 text-right">Revenue</th><th className="px-4 py-2 text-right">Avg/Day</th>
                <th className="px-4 py-2 text-right">Current Stock</th><th className="px-4 py-2 text-right">Days of Stock</th>
              </tr></thead>
              <tbody className="divide-y divide-gray-50">
                {fastMovers.map((m, i) => (
                  <tr key={i} className="hover:bg-gray-50/50">
                    ...
                    <td className="px-4 py-2 text-right">
                      {m.daysOfStock !== null ? (
                        <span className={`font-medium ${m.daysOfStock < 7 ? 'text-red-600' : m.daysOfStock < 14 ? 'text-amber-600' : 'text-green-600'}`}>
                          {m.daysOfStock}d
                        </span>
                      ) : '—'}
                    </td>
      ...
      {activeTab === 'slow' && (
        <div className="card overflow-hidden p-0">
          ...
          <table className="w-full text-sm">
            <thead><tr className="table-header">
              <th className="px-4 py-2">#</th><th className="px-4 py-2">Medicine</th><th className="px-4 py-2">Category</th>
              <th className="px-4 py-2 text-right">Qty Sold</th><th className="px-4 py-2 text-right">Current Stock</th>
