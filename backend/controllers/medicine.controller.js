...
exports.createMedicine = asyncHandler(async (req, res) => {
  ...
  // ── CROWDSOURCED CATALOG: auto-merge into MasterMedicine ──────────
  // One atomic upsert keyed on (medicineName + strength), case- and
  // whitespace-insensitive via collation strength 1. If the (name, strength)
  // pair already exists, this is a no-op — no duplicate, nothing for the
  // SuperAdmin to review. If it's truly new, $setOnInsert seeds the master
  // row from the store's submission so other stores benefit immediately.
  //
  // Wrapped in try/catch so a master-side write failure never blocks the
  // cashier's create flow. Single DB roundtrip — no separate find first.
  try {
    ...
    // E11000 here means a parallel insert won the race — harmless, the
    // row already exists. Anything else is logged but never bubbled up.
    if (err.code !== 11000) {
      console.error('[MASTER-AUTO-MERGE] non-fatal upsert failure:', err.message);
    }
  }
